        ...
        if joined.exists() {
            if let Ok(canonical_target) = joined.canonicalize() {
                if !canonical_target.starts_with(&canonical_base) {
                    return Err(format!("Symlink traversal blocked: path '{}' escapes workspace root", untrusted));
                }
            }
        } else {
            ...
            while let Some(p) = ancestor {
                if p.exists() {
                    if let Ok(canonical_ancestor) = p.canonicalize() {
                        if !canonical_ancestor.starts_with(&canonical_base) {
                            return Err(format!("Symlink traversal blocked: parent directory of '{}' escapes workspace root", untrusted));
                        }
                    }
                    break;
                }
